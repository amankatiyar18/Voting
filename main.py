...
class RBACMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        token = request.cookies.get("access-token") or request.headers.get("Authorization", "").replace("Bearer ", "")
        ...
        if endpoint_path in ("token", "logout", "favicon.ico", "openapi.json", "docs", "To-view-all-api"):
...
@app.post("/refresh")
def refresh_token(request: Request, token_payload: dict = Depends(verify_token)):
    new_token = create_access_token(data=token_payload, expires_delta=datetime.timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES))
    response = JSONResponse(content={"access_token": new_token, "token_type": "bearer"})
    response.set_cookie(key="access-token", value=new_token, httponly=True, secure=True, samesite='Lax')
    return response
...
def create_constituency(constituency: ConstituencyCreate,token_payload: Dict = Depends(verify_token)):
...
def read_all_constituencies(id: Optional[int] = None, name: Optional[str] = None,token_payload: Dict = Depends(verify_token)):
...
def update_constituency_endpoint(constituency_id: int, constituency_updates: ConstituencyCreate,token_payload: Dict = Depends(verify_token)):
...
def delete_constituency(constituency_id: int,token_payload: Dict = Depends(verify_token)):
...
def create_candidate(candidate: CandidateCreate,token_payload: Dict = Depends(verify_token)):
...
def read_all_candidates(id: Optional[int] = None, name: Optional[str] = None, constituency_id: Optional[int] = None, position: Optional[str] = None,token_payload: Dict = Depends(verify_token)):
...
def update_candidate_endpoint(candidate_id: int, candidate_updates: CandidateCreate,token_payload: Dict = Depends(verify_token)):
...
def delete_candidate(candidate_id: int,token_payload: Dict = Depends(verify_token)):
...
def create_result(result: ResultCreate,token_payload: Dict = Depends(verify_token)):
...
def read_all_results(id: Optional[int] = None, name: Optional[str] = None, position: Optional[str] = None,token_payload: Dict = Depends(verify_token)):
...
def update_result_endpoint(result_id: int, result_updates: ResultCreate,token_payload: Dict = Depends(verify_token)):
...
def delete_result(result_id: int,token_payload: Dict = Depends(verify_token)):
...
def create_vote(vote: VoteCreate,token_payload: Dict = Depends(verify_token)):
...
def read_all_votes(id: Optional[int] = None, user_id: Optional[int] = None, candidate_id: Optional[int] = None, result_id: Optional[int] = None,token_payload: Dict = Depends(verify_token)):
